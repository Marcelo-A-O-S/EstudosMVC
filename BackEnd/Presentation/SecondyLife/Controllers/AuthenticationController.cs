                    ...
                    if (result.Succeeded == true)
                    {
                        ...
                        if (result.Succeeded == true)
                        {
                            ...
                            var userData = new UserData
                            {
                                ...
                                Status = Convert.ToInt32(StatusUsuario.online)
                            };
                            if (user.tipoUsuario == TipoUsuario.Administrador)
                            {
                                userData.Role = "Admin";
                            }
                            else if (user.tipoUsuario == TipoUsuario.Usuario)
                            {
                                userData.Role = "User";
                            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel register)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var userValid = await _userManager.FindByEmailAsync(register.Email);
                    if (userValid == null)
                    {
                    ...
                    {
                        return BadRequest("Email já foi registrado no banco de dados,entre com sua conta registrada, ou informe um novo email para cadastro!");
                    }
                }
                return BadRequest();
            }
